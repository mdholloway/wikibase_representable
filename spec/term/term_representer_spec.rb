  ...
  module Representers
    module Term
      describe TermRepresenter do
        let(:term) { Wikibase::Model::Term::Term.new(language_code: 'en', value: 'Douglas Adams') }
        ...
        it 'deserializes a term object' do
          expect(described_class.new(Wikibase::Model::Term::Term.new).from_json(json)).to eq(term)
        end
      end
    end
  end
